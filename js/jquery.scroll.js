	...
	*/
	$.scrollbars.defauls = {
		...
		options: {
			...
			speed			: 200,
			easing			: 'easeOutQuart',
			clase_contenido : '.wrap-content-scroll',
			...
			//
			positionDanguer	: "top", // top - inherit
	...
	$.fn.scrollbars = function( options ){
		...
		return this.each( function(){
			// Opciones
			var wrap	= $( this ).addClass( "wrap-content-overflow" );
			
			// 
			if( typeof options == "string" ){
				//
				return wrap.trigger( options, Array.prototype.slice.call( actions, 1 ) );
			}
			...
			// Establecer opciones 
			var _opt		= $.extend( {}, $.scrollbars.defauls.options, options || {} ),
				settings	= $.extend( {}, _opt,  wrap.data() || {} );
			
			var   
				// Elementos 
				barra = $( document.createElement( "div" ) ).addClass( "scroll-bars" ),
				barrita = $( document.createElement( "div" ) ).addClass( "scroll-baris" ),
				contenido = wrap.find( settings.clase_contenido ).addClass( "wrap-content-scroll" ), 
				...
				// Otras variables 
				factor, ocultar = true,	moveto = new Object(),	offsetTop = 0,			
			...
			setEvent = function() {
				...
				_debug( !settings._plugins.mousewheel ? 
					...
					"$.scrollbars working with \"jquery.mousewheel.min.js\"");
				...
				wrap.bind({
					...
					// Remover scroll
					destroy : endEvent, // remove_scroll
			...
			offsetTopF = function(){
				//
				if( settings.positionDanguer == "inherit" ){
					// top actual
					var topContent = _int( contenido.css( "top" ) ) * -1;
					
					// Calcular posición
					offsetTop = contenido.offset().top + topContent - wrap.offset().top;
			...
			setSize = function () {
				...
				if( _int( settings.bar_size, true ) ){
					...
					// Tamaño dinámico
					barra_height = container_height - offsetTop;
				}
				
				container_height = container_height - offsetTop;
				
				// Asignar alturas.
				barra.setOuterHeight( barra_height - 1 );
			...
			scroll_hide = function () {
				...
				if( settings.hide == true ){
					...
					_debug( !settings._plugins.scroll_activity ? 
						...
						"$.scrollbars working with \"mousewheel_activity.min.js\"");
				}
			},
			...
			onmouse  = function( event, delta, deltaX, deltaY, plugin ) {
				...
				if( contenido.outerHeight() <= wrap.outerHeight() - offsetTop ) return false;	
			...
			barra_click = function( evt ){
				...
				// Mover contenedor 
				return moveto = _moveTo( posicion * factor, undefined, evt );
			},
			...
			onupdate = function(){
				...
				barra_height = barra_height - offsetTop;
				container_height = container_height - offsetTop;
				...
				barrita.setOuterHeight( container_height / factor, false, true );
			...
			_moveTo = function( number, ondrag, evtOrg ){				
				// Variables 
				...
				if( moveto.move ){
					// Evento por defecto
					try {
						if ( !ondrag && evtOrg && evtOrg.preventDefault ) evtOrg.preventDefault();
						else if( !ondrag && evtOrg && evtOrg.returnValue ) evtOrg.returnValue = false;
					} catch( exception ) { };
			...
			_goto = function( evt, ele ){
				...
				if( isPercentage( ele ) ){
					...
					// obtener valores
					moveto = _moveTo( new_val, undefined, evt );
						
				// Comprobar si la $ es un número
				} else if( _int( ele, true ) ){
					// obtener valores
					moveto = _moveTo( ele, undefined, evt );
				...
				// Comprobar si es un elemento
					...
					// obtener valores
					moveto = _moveTo( top - 10, undefined, evt );
				}
			},
